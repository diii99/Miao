...
for p in pois:
    ...
    if tag in ['silver','batik','embroidery']:
        ...
        # Open dancing floor, a small covered stage, and pedestrian approaches.
        box(wood,x,y+.23,z-2.25,4.2,.46,1.4,tag=tag)
        roof(x,y+2.1,z-2.5,4.7,1.8,.55,roofs[0],tag=tag)
        for dx in [-1.9,1.9]: box(dark,x+dx,y+1.15,z-2.5,.14,2.2,.14,tag=tag)
        ...
        box(pathmat,x,y,z,3.5,.18,2.8,tag=tag)
        # Abstract marker, not an asserted reproduction of a real sculpture.
        for i in range(24):
            a=i*math.tau/24
            box(stone,x+math.cos(a)*.85,y+1+math.sin(a)*.85,z-.6,.16,.16,.16,tag=tag)
    elif tag=='terraces':
        pts=[]
        for j in range(81):
